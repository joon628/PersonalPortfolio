...
function initializeDatabase() {
    ...
    db.serialize(() => {
        ...
        Object.keys(initialData).forEach(sectionName => {
            const data = JSON.stringify(initialData[sectionName]);
            
            db.run(`INSERT OR REPLACE INTO portfolio_data 
                   (section_name, data, updated_at, updated_by) 
                   VALUES (?, ?, CURRENT_TIMESTAMP, ?)`,
                   [sectionName, data, 'system'], (err) => {
                if (err) {
                    console.error(`Error inserting ${sectionName} data:`, err.message);
                } else {
                    console.log(`${sectionName} data initialized`);
                }
            });
        });
    });
